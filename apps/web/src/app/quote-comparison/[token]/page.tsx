...
import { agentApi, PublicQuoteComparisonData, RenewalDelta } from '../../../../lib/api';
...
export default function PublicQuoteComparisonPage() {
  const params = useParams();
  const token = String(params.token || '');
  const [data, setData] = useState<PublicQuoteComparisonData | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await agentApi.publicQuoteComparison(token);
        if (!cancelled) setData(r);
      } catch (err: any) {
        if (!cancelled) setError(err?.status === 404 ? 'This quote comparison link is no longer valid.' : (err?.message || 'Could not load quote comparison.'));
      } finally {
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Loading quote comparison...</div>;
  }
  if (error || !data) {
    return (
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 16 }}>
            <div>
              <div style={{ fontSize: 11, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>Current</div>
            ...
            <div>
              <div style={{ fontSize: 11, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>Quote</div>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#0f172a' }}>
        ...
        {visibleDeltas.length === 0 ? (
          <div style={{
            ...
            No tracked field differences between the current policy and this quote.
          </div>
        ) : (
          <div style={{
            backgroundColor: '#fff',
        ...
        <div style={{
          ...
          This summary highlights structured differences between your current policy and a quote your agent
          received. It may miss exclusions, endorsements, or terms not yet shared &mdash; confirm details
          directly with your agent before binding.
        </div>
