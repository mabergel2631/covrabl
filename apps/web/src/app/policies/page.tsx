...
function PoliciesPageInner() {
  ...
  const [personalScore, setPersonalScore] = useState<CoverageScoresResult | null>(null);
  const [businessScore, setBusinessScore] = useState<CoverageScoresResult | null>(null);
  ...
  const loadAll = async () => {
    try {
      ...
      if (scoresResult) {
        setPersonalScore(scoresResult.personal || null);
        setBusinessScore(scoresResult.business || null);
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <section style={{ marginBottom: 40 }}>
          ...
          {!loading && scopedPolicies.length > 0 && (personalScore || businessScore) && (
            <div className="mobile-grid-1" style={{ display: 'grid', gridTemplateColumns: personalScore && businessScore ? '1fr 1fr' : '1fr', gap: 12, marginBottom: 16 }}>
              {[
                { data: personalScore, label: 'Personal Coverage', cats: ['liability', 'property', 'income', 'catastrophic'] as string[] },
                { data: businessScore, label: 'Business Coverage', cats: ['core_liability', 'property_fleet', 'workforce', 'specialty'] as string[] },
              ].filter(t => t.data).map(tile => {
                const d = tile.data!;
        ...
        {error && (
          <div className="alert alert-error" style={{ marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <span>{error}</span>
        ...
        {pendingDrafts.length > 0 && (
          <div style={{ padding: 20, marginBottom: 24, backgroundColor: 'var(--color-success-bg)', border: '1px solid var(--color-success-border)', borderRadius: 'var(--radius-md)' }}>
            ...
            {pendingDrafts.slice(0, 3).map(draft => (
              <div key={draft.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                <span style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>
        ...
        {pendingShares.length > 0 && (
          <div style={{ padding: 20, marginBottom: 24, backgroundColor: 'var(--color-info-bg)', border: '1px solid var(--color-info)', borderRadius: 'var(--radius-md)' }}>
            ...
            {pendingShares.map(s => (
              <div key={s.share_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
